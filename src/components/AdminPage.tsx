...
export const AdminPage: React.FC = () => {
  ...
  const addUser = async (e: React.FormEvent) => {
    ...
    if (!newEmail.trim() || newPassword.length < 6) {
      setError('Informe o e-mail e uma senha de pelo menos 6 caracteres.');
      return;
    }
    setSaving(true);
    setError(null);
    try {
      setAllowed(
        await api.post<AllowedUser[]>('/api/admin/allowed-users', {
          email: newEmail.trim(),
  ...
  const resetPassword = async (email: string) => {
    const pw = prompt(`Nova senha para ${email} (mínimo 6 caracteres):`);
    if (pw === null) return;
    if (pw.length < 6) {
    ...
    try {
      setAllowed(
        await api.patch<AllowedUser[]>(`/api/admin/allowed-users/${encodeURIComponent(email)}`, {
          password: pw,
        }),
      );
    } catch (err: any) {
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                placeholder="pessoa@educacao.pe.gov.br"
                className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-brand-600 focus:ring-1 focus:ring-brand-600"
              />
            </div>
            <div>
              ...
              <input
                ...
                placeholder="mín. 6 caracteres — passe pra pessoa"
                className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-brand-600 focus:ring-1 focus:ring-brand-600"
              />
            </div>
            <div className="flex items-end gap-3">
              ...
              <button
                type="submit"
                disabled={saving}
                className="flex items-center gap-1.5 rounded-lg bg-brand-600 px-4 py-2 text-xs font-bold uppercase tracking-wider text-white hover:bg-brand-700 disabled:opacity-50"
              >
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500">
                <tr>
                  ...
                  <th className="px-4 py-2.5 text-left font-semibold">Último acesso</th>
                ...
                {allowed.map((u) => (
                  <tr key={u.email} className="border-t border-slate-100">
                    <td className="px-4 py-2.5">
                      ...
                      {!u.hasPassword && (
                        <div className="mt-0.5 text-[11px] text-[var(--color-diligencia)]">
                          senha ainda não definida
                        </div>
                      )}
                    </td>
                    <td className="px-4 py-2.5">
